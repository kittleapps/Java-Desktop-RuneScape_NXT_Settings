...
public class BugCorrector {
	...
	private static File js5_16 = new File(Storage.Cache_location+"js5-16.jcache");
	
	public static void CheckKnownBugs() throws IOException {
		if (CheckArdougneCrasher()) {
			int dialogResult = JOptionPane.showConfirmDialog(null, "<html><center>The Ardougne crash bug was detected in your cache!<br>Do you want this program to patch it for you?<br><br>This will patch it by deleting the file 'js5-16.jcache' from your cache directory.","Ardougne crash detected!",JOptionPane.ERROR_MESSAGE);
			...
			try(ResultSet rs = stmt.executeQuery("SELECT * FROM 'cache'")) {
				while (rs.next()) {
					...
					case "431":
						...
						byte[] KnownCorruptedData = {90,76,66,1,0,0,0,45,120,-100,3,0,0,0,0,1};
						DataOutput = "";
						CorruptedDataOutput = "";
						for (byte data : Data) {
							DataOutput = DataOutput+data;
						}
						for (byte data : KnownCorruptedData) {
